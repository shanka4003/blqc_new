...
def gige_camera_worker():
    cameraInfo = Refresh()
    if len(cameraInfo) == 0:
        print("No DVP (GigE) camera found")
        return

    index = 0  # Select the appropriate index
    camera = Camera(index)
    camera.TriggerState = False
    camera.Start()
    window_name = "DVP GigE Camera"
    ...
    while cv2.waitKey(1) != 27:  # ESC to exit
        try:
            ...
            mat = frame2mat(frame)
            if mat is not None:
                cv2.imshow("DVP GigE Camera", mat)
...
def main():
    ...
    while (cv2.waitKey(1) != 27):                           #Press the ESC key to exit the loop
        try:
            ...
            frameInfo, buffer = frame  
            print("uFrameID:{}".format(frameInfo.uFrameID))
        except dvpException as e:
