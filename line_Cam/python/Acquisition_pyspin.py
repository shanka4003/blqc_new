...
def usb_camera_worker():
    system = PySpin.System.GetInstance()
    cam_list = system.GetCameras()

    if cam_list.GetSize() == 0:
        print("No PySpin (USB) camera found")
    ...
    acquisition_mode_continuous = node_continuous.GetValue()
    node_acquisition_mode.SetIntValue(acquisition_mode_continuous)

    cam.BeginAcquisition()

    print("Started USB camera acquisition... Press ESC to exit.")
    window_name = "USB Camera (PySpin)"
    cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)

    # Create ImageProcessor once, outside the loop
    processor = PySpin.ImageProcessor()
    processor.SetColorProcessing(PySpin.SPINNAKER_COLOR_PROCESSING_ALGORITHM_HQ_LINEAR)
    ...
    while True:
        ...
        # Convert to BGR8 using the ImageProcessor
        img_converted = processor.Convert(image, PySpin.PixelFormat_BGR8)
        img_array = img_converted.GetNDArray()
        ...
        key = cv2.waitKey(1)
        if key == 27:  # ESC key
            print("ESC pressed. Exiting USB camera display.")
            break
