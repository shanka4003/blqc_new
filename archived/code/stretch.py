...
# helper that bundles the args
def _flatten_worker(args):
    img, top_pts, bot_pts, out_h = args
    return flatten_strip(img, top_pts, bot_pts, out_h)


def check_alignment(barcode_box, text_box, tolerance=50):
    """
    Check relative alignment of a text box to a barcode box.
    """
    ...
    horizontal_centered = abs(t_cx - b_cx) <= tolerance
    vertical_centered = abs(t_cy - b_cy) <= tolerance
    ...
    if horizontal_centered and above:
        return "centered horizontally above"
    elif horizontal_centered and below:
        return "centered horizontally below"
    elif vertical_centered and left:
        return "centered vertically to the left"
    elif vertical_centered and right:
        return "centered vertically to the right"
    else:
        return "not aligned"
...
def annotate_barcode_and_text(image_path):
    ...
    # --- Barcode detection ---
    h, w = img.shape[:2]
    y0, y1 = int(0.6 * h), int(0.9 * h)
    roi = img[y0:y1, :]

    img_enhanced = enhance_contrast_bgr(roi)
    gray_barcode = cv2.cvtColor(img_enhanced, cv2.COLOR_BGR2GRAY)
    gray_barcode = cv2.GaussianBlur(gray_barcode, (9, 9), 0)
    _, bw = cv2.threshold(gray_barcode, 120, 255, cv2.THRESH_BINARY)

    barcodes = pyzbar.decode(bw)
    for barcode in barcodes:
        x, y, w_box, h_box = barcode.rect
        abs_y = y + y0
        barcode_box = (x, abs_y, w_box, h_box)

        cv2.rectangle(annotated, (x, abs_y), (x + w_box, abs_y + h_box), (0, 255, 0), 2)
        text = f"{barcode.type}: {barcode.data.decode('utf-8')}"
        cv2.putText(annotated, text, (x, abs_y - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        break  # Only use the first barcode for now
    cv2.imwrite('barcode_detection.jpg', bw)
    # --- OCR for batch and expiry ---
    img3 = enhance_contrast_bgr(img)
    cv2.GaussianBlur(img3, (3, 3), 0, img3)
    gray_text = cv2.cvtColor(img3, cv2.COLOR_BGR2GRAY)
    ...
    cv2.imwrite('text_detection.jpg', thresh)
...
# 12/2027|12-2027
        if re.search(r'(?i)(12/2027|12-2027)', word) and expiry_box is None:
            expiry_box = (x, y, w_box, h_box)
            cv2.rectangle(annotated, (x, y), (x + w_box, y + h_box), (255, 0, 0), 2)
            cv2.putText(annotated, "EXP", (x, y - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            # 24747
        elif re.search(r'(?i)24760', word) and batch_box is None:
            batch_box = (x, y, w_box, h_box)
            cv2.rectangle(annotated, (x, y), (x + w_box, y + h_box), (255, 255, 0), 2)
            cv2.putText(annotated, "BATCH", (x, y - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)

        if batch_box and expiry_box:
            break

    # --- Alignment Check ---
    if barcode_box and batch_box:
        batch_alignment = check_alignment(barcode_box, batch_box)
        print("Batch Alignment:", batch_alignment)
    else:
        print("Batch or barcode not found")
        batch_alignment = None

    if barcode_box and expiry_box:
        expiry_alignment = check_alignment(barcode_box, expiry_box)
        print("Expiry Alignment:", expiry_alignment)
    else:
        print("Expiry or barcode not found")
        expiry_alignment = None

    return annotated, batch_alignment, expiry_alignment

# ─── your full pipeline below ─────────────────────────────────────────────────

def compensate_with_flow(img1, img2, pyr_scale=0.5, levels=2, winsize=30):
    ...
    flow = cv2.calcOpticalFlowFarneback(
        g2, g1, None,
        pyr_scale,      # maybe 0.3 instead of 0.5
        levels,         # try 1 instead of 2
        winsize,        # 40 or lower instead of 80
        3,              # iterations: 2 or 1
        3,              # poly_n: try 3
        1.1,            # poly_sigma: try 1.0
        0               # flags
    )
    half = flow * 0.5
    h, w = g1.shape
...
def estimate_horizontal_shift_with_bf(img_ref, img_new, mean_shift, ratio_thresh=0.9):
...
def stitch_pair_by_simple_blend(img_base, img_new, shift):
    h_b, w_b = img_base.shape[:2]
    h_n, w_n = img_new.shape[:2]
    ...
    x_n = x_b + shift
    start = max(x_b, x_n)
    end = min(x_b + w_b, x_n + w_n)

    if end <= start:
        # No overlap — just place both images
        pano[:h_n, x_n:x_n+w_n] = img_new
        return pano

    # Blending in overlapping region
    overlap_width = end - start
    alpha = np.linspace(0, 1, overlap_width).reshape(1, overlap_width, 1)

    base_crop = img_base[:, (start - x_b):(end - x_b)].astype(np.float32)
    new_crop  = img_new[:, (start - x_n):(end - x_n)].astype(np.float32)
    blended = (base_crop * (1 - alpha) + new_crop * alpha).astype(np.uint8)
...
def stitch_strips_with_flow(strips):
    ...
    for i in range(1, len(strips)):
        cur = strips[i]
        mean_s = np.mean(prevshifts[2:-1]) if len(prevshifts)>3 else 100
        bf  = estimate_horizontal_shift_with_bf(prev, cur, mean_s)
        shift = refine_shift_with_flow(prev, cur, -bf)
        prevshifts.append(shift)
        #run stitching pair by shift if the simple funstion if not performing well
        # pano = stitch_pair_by_shift(pano, cur, sum(prevshifts))
        pano = stitch_pair_by_simple_blend(pano, cur, sum(prevshifts))
        prev = cur
...
class ProcessThread(QThread):
    ...
    def __init__(self,
        ...
        self.get_burst_size = burst_size
            ...
            if self.input_type == "Auto Mode":
                ...
                try:
                    ...
                    while True:
                        ...
                        # wait for ROI change
                        time.sleep(15)  # wait a bit before checking
                        while True:
                        ...
                        for i in range(self.get_burst_size()):
                            f = cam.GetNextImage()
                            if not f.IsIncomplete():
                                burst.append(f.GetNDArray().copy())
                            f.Release()
                            self.progress.emit(int(100 * (i+1) / self.get_burst_size()))
                        ...
                        # strips = burst
                        tasks = [(frame, top, bot, self.height) for frame in burst]
                        with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
                            strips = list(executor.map(_flatten_worker, tasks))
                        if self.no_stitch:
                            pano = cv2.hconcat(strips)
                        else:
                            pano = stitch_strips_with_flow(strips)
                        duration = time.time() - start_time
                        print(f"[Total Stitching Time] {duration:.3f} seconds")
                        # cam.EndAcquisition()
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        btnAutoPreview = QPushButton("Preview Camera & Select Points"); btnAutoPreview.clicked.connect(self.preview_camera_and_pick_points)
        ...
        self.batchAlignLabel = QLabel("Batch Alignment: N/A")
        self.expAlignLabel   = QLabel("Expiry Alignment: N/A")
        self.autoModeLabel = QLabel("🟠 Auto Mode Active")
        self.autoModeLabel.setStyleSheet("color: red; font-weight: bold;")
        self.autoModeLabel.setAlignment(Qt.AlignCenter)
        self.autoModeLabel.setVisible(False)  # Hidden by default
        # ── Layout ─────────────────────────────────────────────────────────
        ...
        self.chkAnalyze = QCheckBox("Detect Barcode + Batch/Exp")
        self.chkAnalyze.setChecked(True)
        ...
        ctrlLay.addWidget(btnAutoPreview)
        ctrlLay.addWidget(self.chkAnalyze)
        mainLay = QVBoxLayout()
        ...
        mainLay.addWidget(self.batchAlignLabel)
        mainLay.addWidget(self.expAlignLabel)
        mainLay.addWidget(self.autoModeLabel)
    ...
    def run_process(self):
        path, pts = self.inputEdit.text().strip(), self.ptsEdit.text().strip()
        if self.input_type != "Auto Mode":
            self.autoModeLabel.setVisible(True)
        ...
        if self.input_type == "Auto Mode":
            args += [
                ...
                lambda: self.captureCountSpin.value()
            ]
    ...
    def on_done(self, pano):
        ...
        out_path = ""
        if self.input_type == "Frames Folder":
            out_path = os.path.join(self.inputEdit.text(), "label_panorama.jpg")
        elif self.input_type == "Auto Mode":
            out_path = os.path.join(os.getcwd(), "auto_panorama.jpg")
        else:
            d, fn = os.path.split(self.inputEdit.text())
            base, _ = os.path.splitext(fn)
            out_path = os.path.join(d, f"{base}_panorama.jpg")

        cv2.imwrite(out_path, pano)

        # Perform barcode & text analysis if enabled
        if self.chkAnalyze.isChecked():
            annotated, batch_alignment, expiry_alignment = annotate_barcode_and_text(out_path)
            cv2.imwrite(out_path, annotated)
        else:
            annotated = pano
            batch_alignment = expiry_alignment = ""
        self.batchAlignLabel.setText(f"Batch Alignment: {batch_alignment}")
        self.expAlignLabel.setText(f"Expiry Alignment: {expiry_alignment}")
        
        h, w, ch = annotated.shape
        img = QImage(annotated.data, w, h, ch * w, QImage.Format_BGR888)
        self.imageLabel.setPixmap(
            QPixmap.fromImage(img)
                .scaled(self.imageLabel.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
        )
        
        duration = time.time() - start_time
        print(f"[Total Stitching Time] {duration:.3f} seconds")
        self.progress.setVisible(False)
        # self.autoModeLabel.setVisible(False)
    ...
    def preview_camera_and_pick_points(self):
        # if hasattr(self, "thread") and self.thread.isRunning():
        #     QMessageBox.warning(self, "Error", "Stop Auto Mode before previewing camera.")
        #     return

        pts_path = self.ptsEdit.text().strip()
        if not pts_path:
            QMessageBox.warning(self, "Error", "Invalid points path.")
            return
        ...
        try:
            ...
            img = cam.GetNextImage()
            if img.IsIncomplete():
                raise RuntimeError("Image is incomplete.")
            frame = img.GetNDArray().copy()

            tmp = tempfile.NamedTemporaryFile(suffix=".jpg", delete=False)
            cv2.imwrite(tmp.name, frame)
            tmp.close()
            select_points(tmp.name, pts_path)
            os.unlink(tmp.name)
            ...
            try:
                ...
                QMessageBox.warning(self, "Cleanup Warning", f"ReleaseInstance failed:\n{e}")
